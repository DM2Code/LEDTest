...
def main():
  ...
  # for cc in [[255,160,240],[255,255,255]]:
  for cc in [[255,160,240]]:
    with ioqlock:
      ioreqs.append(setColorCorrectionBytes(RGB(cc[0],cc[1],cc[2])))
    time.sleep(0.5)

  # End on high contrast
  ledqt.thlog_info("     ***  High contrast between sections - R G B R G B R G B ... ***     ")
    ...
    with ioqlock:
      for i in range(NUM_CHANNELS):
        ioreqs.append(setChannelBytes(i, _style=LS_ON,
                        _col1=RGB(mask[(NUM_CHANNELS+0-i)%3]*255,
                                  mask[(NUM_CHANNELS+1-i)%3]*255,
                                  mask[(NUM_CHANNELS+2-i)%3]*255 )))
    time.sleep(1.5)
